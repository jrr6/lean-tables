...
def List.unique {α} [DecidableEq α] (xs : List α) := reverse (uniqueAux xs [])

-- Equivalent to List.filterMap id
def List.somes : List (Option α) → List α
| [] => []
| none :: xs => somes xs
| (some x) :: xs => x :: somes xs

-- def Row.fromLabeledRowList {η} [DecidableEq η] {τ}:
--   (lrs : List $ Row [(_, η), (_, τ)]) → Row $ (lrs.map (λ (r : Row [(_, η), (_, τ)]) => Cell.toOption (r.getCell Schema.HasCol.hd))).somes.unique.map (λ nm => (nm, τ)) := sorry

def Row.fromLabels :
  (ps : List (η × Option τ)) → Row (ps.map (λ p => (p.1, τ)))
| [] => Row.nil
| p :: ps => Row.cons (Cell.fromOption p.2) (fromLabels ps)

def Row.fromCellList {nm : η} {τ} : (xs : List (Cell nm τ)) → Row _
| [] => Row.nil
| c :: cs => Row.cons c (fromCellList cs)

def pivotWider (t : Table schema)
  (c1 : (c : η) × Schema.HasCol (c, η) schema)
  (c2 : CertifiedHeader (schema.removeHeader c1.2))
  [DecidableEq c2.1.2]
  -- TODO: chances are this won't be inferrable and we'll need to
  -- make this explicit and have the client use `inst`
  [DecidableEq $
    Row (Schema.fromCHeaders (List.map (λ ch =>
          ⟨ch.1, Schema.removeHeaderPres (Schema.removeHeaderPres ch.snd)⟩)
        (Schema.certify (schema.removeHeaders
          (ActionList.cons ⟨(c1.fst, η), c1.snd⟩
            (ActionList.cons c2 ActionList.nil))))))]
    : Table (List.append
      (schema.removeHeaders $ ActionList.cons ⟨(c1.1, η), c1.2⟩
                                (ActionList.cons c2 ActionList.nil))
      ((getColumn2 t c1.1 c1.2).somes.unique.map (λ nm => (nm, c2.val.2)))) :=
  let keptColumns : Schema := schema.removeHeaders $
    ActionList.cons ⟨(c1.1, η), c1.2⟩ (ActionList.cons c2 ActionList.nil)
  let newColumns : Schema := (getColumn2 t c1.1 c1.2).somes.unique.map (λ nm =>
    (nm, c2.val.2))
  let newColumnsCertified := Schema.certify (List.append keptColumns newColumns)
  groupBy t
    (λ r => r.pick $
      keptColumns.certify.map (λ ch =>
        ⟨ch.1, Schema.removeHeaderPres (Schema.removeHeaderPres ch.2)⟩))
    -- TODO: I don't think this works because the pivoted-cell row order isn't
    -- necessarily guaranteed to be consistent
    -- (λ r => r.getCell $ Schema.removeHeaderPres c2.2)
    (λ r => r.pick [⟨(c1.1, η), c1.2⟩, ⟨c2.1, Schema.removeHeaderPres c2.2⟩])
    (λ k vs =>
      -- k : Row (schema - c.1 - c.2) -- i.e., the common row entries that are
      -- duplicated by `pivotLonger`
      -- vs : List (Row [(c1, η), (c2, τ)]) -- i.e., a list of the key-value
      -- pairs generated by `pivotLonger` for this common row


      -- Rough approximation of B2T2 approach (likely not translatable)
      -- let lookup {sch} (t : Table sch) (c) (v) :=
      --   let filtered := tfilter t (λ r => decide $ getValue r c = v)
      --   if h : nrows filtered > 0
      --   then getRow filtered 0 h
      --   else Row.empty sch
      -- Row.append k (newColumns.map (λ c =>
      --   Row.getCell (lookup (values vs) c1 c) c2
      -- ))

      -- TODO: can't be a map because we need to throw away everything that's
      -- got an empty first cell (this actually instantiates η_{fromLabels} to
      -- Cell nm η)
      let x := Row.fromLabels (vs.map (λ v =>
        (v.getCell Schema.HasCol.hd, Cell.toOption $ v.getCell $ Schema.HasCol.tl Schema.HasCol.hd)
      ))
      sorry
    )
